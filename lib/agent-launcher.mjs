...
export function createLauncherCore({
  ...
  async function launchAgent({ provider, model, executionMode, agentId, workdir, responseLog } = {}) {
    ...
    // BL-024 T3b: model is a required companion to vendor for goose — goose is a harness over an
    // arbitrary OpenRouter model, so `goose` alone names no agent. Reject early (before the
    // orchestrator create or any spawn). claude/gemini/codex keep sensible model defaults downstream.
    if (resolvedProvider === 'goose' && (typeof model !== 'string' || model.trim() === '')) {
      throw new AgentLauncherError('model is required for goose (it is a harness over an OpenRouter model)', 400);
    }
    // BL-024 T3a/T3b: the orchestrator wire uses the transport/vendor axis. All four canonical
    // vendors (incl. goose as of T3b) attach as `{transport:'attached', vendor}`; `agy` already
    // resolved to `gemini` above. (The legacy `provider` field is dropped from the wire in T3b-2.)
    const agentKind = { transport: 'attached', vendor: resolvedProvider };
    ...
    // BL-082: a launched worker is not a human session, and must not be caught by the turn-1
    // primer gate. The AgentTalk repo's `.claude/settings.json` SessionStart hook tells a Claude
    // session to do the primer handshake, report and STOP — which for an autonomous worker means
    // doing nothing while looking healthy. That hook is already guarded by
    // `[ -n "$AGENTTALK_SKIP_PRIMER" ]`; nothing set it, so the guard had never been exercised.
    // (BL-080 seemed to clear this, but ran in a worktree where `.claude/` is gitignored and the
    // hook cannot fire; in the PRIMARY checkout it can.) Set for every provider: the signal is
    // "launched worker", not "claude".
    const env = {
      ...process.env,
      AGENTTALK_PERSISTENT_MCP_URL: mcpUrl,
      AGENTTALK_WORKDIR: workdir,
      AGENTTALK_SKIP_PRIMER: '1',
    };
