...
// The `agy` (Antigravity) binary is a different product from the `gemini` CLI and
// does NOT read gemini's `.gemini/settings.json` / GEMINI_CLI_HOME. It discovers
// MCP servers from a plugin in a customization root: `.agents/plugins/<name>/`
// with a `plugin.json` marker plus an `mcp_config.json`, relative to the workspace
// it is spawned in. See BL-045 / LB-93.
const AGY_BRIDGE_PLUGIN_NAME = 'agenttalk-bridge';

export function writeAgyBridgePlugin(workspaceDir, agentMcpUrl) {
    const pluginDir = join(workspaceDir, '.agents', 'plugins', AGY_BRIDGE_PLUGIN_NAME);
    mkdirSync(pluginDir, { recursive: true });
    // plugin.json is the marker that declares the directory a plugin.
    writeFileSync(join(pluginDir, 'plugin.json'), JSON.stringify({ name: AGY_BRIDGE_PLUGIN_NAME }, null, 2));
    writeFileSync(join(pluginDir, 'mcp_config.json'), JSON.stringify({
        mcpServers: {
            bridge: {
                command: 'node',
                args: [bridgePath, agentMcpUrl],
...
class GeminiPersistentExecutor extends BasePersistentExecutor {
    ...
    async executeTurn(request, sink = {}) {
        if (process.env.AGENTTALK_PERSISTENT_MCP === 'true') {
            ...
            // agy discovers `.agents/` relative to the workspace it is spawned in, and
            // sink.cwd is per-turn (the orchestrator may name a different one), so the
            // plugin is written against the cwd actually used. Idempotent by design.
            const workspaceDir = sink.cwd || process.cwd();
            try {
                writeAgyBridgePlugin(workspaceDir, this._agentMcpUrl);
            }
            catch (err) {
                console.error(`[GeminiPersistentExecutor] Failed to write the bridge plugin into ${workspaceDir}:`, err);
            }
