...
import { callProvider, workerGitIdentityEnv } from './provider-runtime.mjs';
...
function getPersistentProviderCommand(providerName, selectedModel) {
    if (providerName === 'claude') {
        return {
            ...
            // BL-102: this path does NOT go through getSpawnEnv, and it is the path that
            // produced the defect (H-L3's worker was claude/persistent). Injecting only at
            // getSpawnEnv would have gone green while every real worker commit stayed
            // authored as the human. Copy, never `process.env` by reference.
            env: { ...process.env, ...workerGitIdentityEnv(providerName) },
        };
    }
            ...
            return new Promise((resolve, reject) => {
                ...
                const proc = spawn(baseCmd.command, args, {
                    ...
                    env: {
                        ...baseCmd.env,
                        ...process.env,
                        // BL-102: after ...process.env, so the identity cannot be shadowed by
                        // an inherited GIT_AUTHOR_*/GIT_COMMITTER_* from the parent process.
                        ...workerGitIdentityEnv('gemini'),
    ...
    async executeTurn(request, sink = {}) {
        {
            ...
            return new Promise((resolve, reject) => {
                ...
                const proc = spawn('codex', args, {
                    ...
                    cwd: sink.cwd || process.cwd(),
                    // BL-102: copy + worker identity. Was `process.env` by reference.
                    env: { ...process.env, ...workerGitIdentityEnv('codex') },
                    stdio: ['ignore', 'pipe', 'pipe'],
                });
                proc.stdout?.on('data', (chunk) => {
                    const text = chunk.toString();
