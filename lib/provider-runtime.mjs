...
/**
 * BL-102: the four git identity variables for a launched worker.
 *
 * Every autonomous worker commit back to O-1 was authored as the human, because git resolves
 * identity from the machine and the worker is a process on the PO's machine. In git history
 * agent work was therefore indistinguishable from the PO's own.
 *
 * This is set on the PROCESS, not on a directory, and that is deliberate: where a worker
 * commits DEPENDS ON ITS EXECUTION PATH. claude/persistent has a session-level cwd (the
 * assigned workdir) and structurally cannot take the per-turn task dir; gemini, codex and
 * every one-shot commit inside the provisioned task worktree. A directory-scoped fix would
 * have to be applied in two unrelated places and would still miss whichever path was
 * overlooked. Env follows the process wherever it commits.
 *
 * It also avoids a live footgun: `git config` inside a LINKED WORKTREE is not worktree-scoped
 * — it writes the shared common config, so setting an agent identity there rewrites the
 * PRIMARY checkout's identity too, and the human's own later commits would be authored as an
 * agent. Nothing here writes any git config.
 *
 * COMMITTER is set as well as AUTHOR: `--author` alone leaves the committer field claiming a
 * human, and that field survives a rebase.
 */
export function workerGitIdentityEnv(providerName) {
    const agentId = process.env.AGENTTALK_AGENT_ID || 'unknown';
    const name = `AgentTalk worker (${providerName || 'unknown'})`;
    const email = `agent+${agentId}@agenttalk.local`;
    return {
...
export function getSpawnEnv(providerName) {
    // BL-102: always a COPY. This previously returned `process.env` BY REFERENCE for every
    // non-claude provider and a copy only for claude — so a caller adding to the returned
    // object would mutate this process's own environment for some providers and not others.
    // Declared as a deliberate change to shared logic, not a drive-by: the identity below
    // must not leak into the llm-agent's own env.
    const env = { ...process.env, ...workerGitIdentityEnv(providerName) };
...
export async function callProvider(providerName, selectedModel, userMessage, options = {}) {
    ...
    // BL-075: `cwd` is the per-turn task worktree the orchestrator assigned, threaded down
    // from the executor's sink. Undefined means no task dir was provisioned — then the child
    // inherits our own cwd (the assigned workdir), which is the pre-BL-075 behaviour.
