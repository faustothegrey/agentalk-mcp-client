...
export function createBite0Runner(deps) {
  ...
  async function run(config) {
    ...
    try {
      ...
      // 7. release the worker's task worktrees (BL-103). OPTIONAL dep — absent means no-op, so
      // every existing caller behaves exactly as before. Best-effort like the recorder: cleanup
      // that can fail a run is worse than the leak it fixes.
      //
      // It runs BEFORE stopInstance and inside `finally`, so a capped or errored run is cleaned up
      // too — those are precisely the runs nobody is watching. It cannot destroy work: see
      // releaseTaskDirs, which uses `worktree remove` without --force and `branch -d` never `-D`.
      if (typeof deps.releaseTaskDirs === 'function') {
        try {
          const released = await deps.releaseTaskDirs(config.agents[0]);
          if (released) emit({ event: 'task-worktrees-released', ...released });
        } catch (e) {
          logger.error?.(`[bite0] releaseTaskDirs failed: ${e?.message}`);
          emit({ event: 'task-worktrees-released', errors: [{ step: 'call', detail: e?.message ?? String(e) }] });
        }
      }
